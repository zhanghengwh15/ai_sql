...
### 2.2 字段规范
- 【强制】后端业务表字段（含时间戳字段）默认不能为 NULL，且都需要有默认值（NOT NULL DEFAULT XXX）
- 【推荐】中台表字段除主键、必要业务唯一标识、状态/审计字段、明确参与必填校验或处理流程控制的字段外，一般默认允许 NULL；不确定上游是否稳定返回时优先使用 NULL 表达“缺失/未提供”，避免用 0、空字符串或哨兵时间伪造业务值
...
- 【强制】中台对接表不要求org_id，但必须包含eid字段，类型为varchar(64)
- 【强制】中台对接表承接外部系统数据，除id、明确必填字段、审计/状态等内部控制字段外，其他外部数据字段默认允许NULL，避免对接数据缺失或形态不确定导致入库失败
...
- 定期分析慢查询日志，优化性能瓶颈
- 监控索引碎片率，及时维护索引
- 后端多租户环境下，确保所有查询都包含org_id条件，避免跨租户数据泄露；中台对接表按eid识别企业数据，不强制org_id

### 8.2 多租户安全考虑
- 【强制】后端多租户业务查询必须包含org_id条件，确保数据隔离
- 【强制】后端多租户索引设计必须考虑租户隔离，org_id作为第一列
- 【强制】后端多租户唯一性约束必须包含org_id，确保跨租户数据独立性
- 【强制】中台对接表不要求org_id，必须使用eid（varchar(64)）承接企业标识
- 【强制】定期检查后端多租户查询是否有遗漏org_id条件；中台对接查询是否有遗漏eid条件
...
- 后端多租户环境下，所有SQL查询必须包含org_id条件；中台对接表查询按业务需要使用eid条件
