...
4. **性能优化** - 字段类型优化、查询模式优化，以及上线后的索引评估
...
- **约束设计**: 后端业务表默认使用 NOT NULL；中台表除非必须，一般默认允许 NULL；初始化建表不创建会隐式生成索引的UNIQUE约束
- **默认值**: 后端业务表设置合理默认值；中台表仅对必须非空的字段设置默认值
- **注释**: 为每个字段添加清晰的注释说明
...
- **初始化索引策略**:
  - `CREATE TABLE` 只创建主键索引
  - 不创建普通索引、唯一索引、全文索引等业务索引
  - 不在初始化建表脚本中附带候选索引或 `ALTER TABLE ... ADD KEY` 语句
  - 表上线后再依据真实查询、慢查询日志和 `EXPLAIN` 单独设计索引变更
...
- **设计思路**: 说明设计决策和考虑因素
- **索引说明**: 初始化设计只说明主键索引；不输出业务索引清单或候选索引SQL
...
- **全文索引**: 表上线后确有文本搜索需求时，再单独评估FULLTEXT索引
- **生成列**: 使用虚拟列或存储列简化查询
...
2. **字段规范**: 后端业务表字段必须设置 NOT NULL 和默认值；中台表字段除主键、必要业务唯一标识、状态/审计字段、明确参与必填校验或处理流程控制的字段外，一般默认允许 NULL
...
- [ ] 后端业务表字段是否设置了NOT NULL和合理的默认值；中台表字段是否除必须非空字段外默认允许NULL
- [ ] 表名和字段名是否符合命名规范
...
4. 初始化建表只保留主键索引，不生成业务索引或候选索引SQL
