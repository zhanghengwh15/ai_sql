...
-- 日期：    2026-08-20
-- 计算规则：
--   1. 在同一 eid 内，按 dop_modify_time、id 排序；
--   2. 当前记录相对紧邻上一条记录发生变化时：
--      - doc_no 或 status 发生变化：cal_status = 1；
--      - doc_no、status 均一致，仅 dop_modify_time 变化：cal_status = 2；
--   3. 同一 eid 只有一条有效记录时：
--      - status = 0：cal_status = 2；
--      - status <> 0 或 status IS NULL：cal_status = 1；
--   4. doc_no、status、dop_modify_time 均未变化时：cal_status = 2。
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- 1. 范围表：锁定每个 eid 当前最新且待计算的记录
--    - cal_status = 0：幂等条件
--    - modify_time：只处理最近 24 小时发生变化的数据
--    - NOT EXISTS：排除同一 eid 下比当前记录更新的有效记录
-- ---------------------------------------------------------------------
...
SELECT
    ...
    CASE
        ...
        WHEN NOT (
            (`history_order`.`prev_doc_no` <=> `history_order`.`doc_no`)
            AND
...
SELECT
    ...
    `staging_order`.`prev_doc_no`,
    `staging_order`.`doc_no`,
