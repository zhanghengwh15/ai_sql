...
WHERE `calculate_status` = 1
    AND (
        `work_order_number` LIKE 'CW%'
        OR `work_order_number` LIKE 'DW%'
    )
...
-- ---------------------------------------------------------------------
-- 2. 按物料编码补全默认仓库和物料类型；物料正式表未命中时保留原值
-- ---------------------------------------------------------------------
...
SET `work_order`.`moren_warehouse_id` = `material`.`moren_warehouse_id`,
    `work_order`.`materialtypecode` = `material`.`materialtypecode`,
    `work_order`.`modify_time` = NOW()
WHERE `work_order`.`calculate_status` = 1;

/* 验证 SQL-2：核对默认仓库、物料类型补全与物料未命中记录
SELECT
    `work_order`.`moren_warehouse_id`,
    `work_order`.`materialtypecode`,
    COUNT(*) AS `record_count`
FROM `yt_mid_produce_work_order` AS `work_order`
INNER JOIN `yt_mid_produce_work_order_scope_tmp` AS `scope`
    ON `scope`.`id` = `work_order`.`id`
GROUP BY `work_order`.`moren_warehouse_id`, `work_order`.`materialtypecode`;
...
-- ---------------------------------------------------------------------
-- 3. 按优先级计算门店编码
--    1) DW 开头的低温工单：按物料类型设置，未匹配时为 810
--    2) CW 开头的常温工单：trace_number 非空时为 870
--    3) CW 开头的常温工单：默认仓库为 tbYAAAy7mTy76fiu 时为 821
--    4) 其他 CW 开头的常温工单：820
-- ---------------------------------------------------------------------
UPDATE `yt_mid_produce_work_order` AS `work_order`
INNER JOIN `yt_mid_produce_work_order_scope_tmp` AS `scope`
...
SET `work_order`.`store_code` = CASE
        WHEN `work_order`.`work_order_number` LIKE 'DW%' THEN CASE
            WHEN `work_order`.`materialtypecode` = '1101' THEN '831'
            WHEN `work_order`.`materialtypecode` = '1105' THEN '863'
            WHEN `work_order`.`materialtypecode` = '1106' THEN '864'
            ELSE '810'
        END
        WHEN `work_order`.`work_order_number` LIKE 'CW%' THEN CASE
            WHEN `work_order`.`trace_number` IS NOT NULL THEN '870'
            WHEN `work_order`.`moren_warehouse_id` = 'tbYAAAy7mTy76fiu' THEN '821'
            ELSE '820'
        END
        ELSE `work_order`.`store_code`
    END,
    `work_order`.`modify_time` = NOW()
WHERE `work_order`.`calculate_status` = 1;
...
SELECT `work_order`.`id`, `work_order`.`trace_number`,
       `work_order`.`work_order_number`, `work_order`.`moren_warehouse_id`,
       `work_order`.`materialtypecode`, `work_order`.`store_code`
FROM `yt_mid_produce_work_order` AS `work_order`
INNER JOIN `yt_mid_produce_work_order_scope_tmp` AS `scope`
    ON `scope`.`id` = `work_order`.`id`
ORDER BY `work_order`.`id`
